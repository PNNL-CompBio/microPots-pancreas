...
p4=plot_pca(m2, phenotype = "IsletStatus",z_score=T)+ggtitle("Scaled data by location") #check biplot = T?

p6=plot_pca(m2, phenotype = "IsletNumber",z_score=T)+ggtitle("Scaled data by Islet") #check biplot = T?
...
names(navals)<-all.cols
 ...
 library(UpSetR)
 pdf('diffExOverlap.pdf',onefile=F,height=4)
 upset(as.data.frame(comb.res),sets=all.cols)
 dev.off()
# ggsave('diffExOverlap.pdf',p,width=10)
...
p
ggsave('gseaResultsIslet.pdf',width=10)

top.set.res<-leapR(datamatrix=full.islet,krbpaths,'enrichment_in_sets',primary_columns='logFC',minsize=10,
                   threshold=top.logfc,greaterthan=TRUE)%>%
  subset(BH_pvalue<0.05)%>%
  subset(oddsratio>0)%>%
  dplyr::select('ingroup_n','ingroup_mean','oddsratio','BH_pvalue')%>%
  arrange(BH_pvalue)

p1<-plotLeapR(top.set.res,'REACTOME')
p1

bottom.set.res<-leapR(datamatrix=full.islet,krbpaths,'enrichment_in_sets',primary_columns='logFC',
                      minsize=10,
                      threshold=bottom.logfc,greaterthan=FALSE)%>%
  subset(BH_pvalue<0.05)%>%
  subset(oddsratio<0)%>%
  dplyr::select('ingroup_n','ingroup_mean','oddsratio','BH_pvalue')%>%
  arrange(BH_pvalue)

p2<-plotLeapR(bottom.set.res,'REACTOME')
