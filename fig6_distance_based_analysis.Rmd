...
p2<-with.signif%>%
  ...
  geom_jitter()+
  theme(axis.text.x=element_blank())+ggtitle("Significance of insulin signaling proteins")
...
plotPathwayCors<-function(krb.order.tab,pathname){
  ...
  p2<-with.signif%>%
    ggplot(aes(x=factor(To,levels=cor.means$To),y=corVal,col=as.factor(`Islet Number`),shape=signif))+
    geom_jitter()+
    theme(axis.text.x=element_blank())
  
#  p2

  ggsave(paste0('allCorrelations',pathname,'.pdf'),p2,width=10)
